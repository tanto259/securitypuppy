...
For the COBOL client program, I utilized [EXEC CICS LINK](https://www.ibm.com/docs/en/cics-ts/5.5?topic=interface-exec-cics-link-command-exci), passing data via a communication area. Make sure that when control is transferred to the CICS program, it will not encounter any of the [prohibited API commands](https://www.ibm.com/docs/en/cics-ts/5.5?topic=link-exception-conditions-command), which include RECEIVE and SEND.
